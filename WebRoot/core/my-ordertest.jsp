			...
			<div class="personal_r_m_m ">
				...
				<div class="coll yellow">
					<dl> 
						...
						<%-- <dd><a href="UserServlet?method=SelectOrdersServlet&userid=${userid}&Status=1" class="blue">已发货<span class="yellow"></span></a></dd> --%>
						<dd><a href="UserServlet?method=SelectOrdersServlet&userid=${userid}&Status=2" class="blue">未付款<span class="yellow"></span></a></dd> 
						<%-- <dd><a href="UserServlet?method=SelectOrdersServlet&userid=${userid}&Status=3" class="blue">已付款<span class="yellow"></span></a></dd>  --%>
						<dd><a href="UserServlet?method=SelectOrdersServlet&userid=${userid}&Status=4" class="blue">未收货<span class="yellow"></span></a></dd> 
						<%-- <dd><a href="UserServlet?method=SelectOrdersServlet&userid=${userid}&Status=5" class="blue">已收货<span class="yellow"></span></a></dd> --%>
						...
						<tbody>
							<c:forEach items="${selectOrders}" var="orders">
								<tr>
									...
									<td>
									<c:if test="${orders.status eq 1}"><a href="UserServlet?method=SelectOrdersByuserIdAndOrderId&userid=${userid}&orderId=${orders.id}&statu=${orders.status}" class="red">[查看]</a></c:if> 
									<c:if test="${orders.status eq 2}"> <a href="#" class="yellow">[支付]</a>|<a onclick="delOrder(${orders.id})" class="blue">[删除订单]</a> </c:if>
									<c:if test="${orders.status eq 0}"> <a onclick="delOrder(${orders.id})" class="blue">[取消]</a></c:if>																								<!--  -->			
									<c:if test="${orders.status eq 3}"> <a href="UserServlet?method=SelectOrdersByuserIdAndOrderId&userid=${userid}&orderId=${orders.id}&statu=${orders.status}" class="blue">[查看]</a></c:if>
									...
									<c:if test="${orders.status eq 5}"><a href="UserServlet?method=SelectOrdersByuserIdAndOrderId&userid=${userid}&orderId=${orders.id}&statu=${orders.status}" class="red">[查看]</a></c:if>
